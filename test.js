...
async function runTest() {
  ...
  try {
    ...
    if (!assert(loginPageResponse.ok, `Login page responded with status ${loginPageResponse.status}`)) {
      testsFailed++;
    ...
    if (!assert(loginPageHtml.length > 1000, `Login page HTML length: ${loginPageHtml.length} bytes`)) {
      testsFailed++;
    ...
    if (!assert(csrfToken !== null, `CSRF token extracted: ${csrfToken ? csrfToken.substring(0, 20) + '...' : 'NULL'}`)) {
      testsFailed++;
      logError('CSRF token not found in HTML. The form structure may have changed.');
    ...
    if (!assert(publicKey !== null, `RSA public key extracted: ${publicKey ? 'YES' : 'NO'}`)) {
      testsFailed++;
      logError('RSA public key not found in HTML. The JavaScript may have changed.');
      logError('Look for: -----BEGIN PUBLIC KEY-----');
      throw new Error('RSA public key missing');
    }
    testsPassed++;

    if (!assert(publicKey.includes('BEGIN PUBLIC KEY'), 'RSA key format valid')) {
      testsFailed++;
    ...
    if (!assert(loginResponse.status === 302, `Login response status: ${loginResponse.status} (expected 302 redirect)`)) {
      testsFailed++;
      if (loginResponse.status === 200) {
        logError('Got 200 instead of 302. Login may have failed - check credentials or form structure.');
      }
    ...
    if (!assert(cookies.length > 0, `Session cookies received: ${cookies ? 'YES' : 'NO'}`)) {
      testsFailed++;
      logError('No cookies in login response. Session management may have changed.');
    ...
    if (!assert(redirectLocation !== null, `Redirect location: ${redirectLocation || 'NONE'}`)) {
      testsFailed++;
    ...
    if (!assert(targetResponse.ok, `Target page status: ${targetResponse.status}`)) {
      testsFailed++;
    ...
    if (!assert(targetHtml.length > 5000, `Target page HTML length: ${targetHtml.length} bytes`)) {
      testsFailed++;
      logWarning('Target page HTML seems too short. May not be the expected page.');
    } else {
      testsPassed++;
    }
    ...
    if (!assert(typeof parsed.summary?.overallAverage === 'number', `Overall average: ${parsed.summary?.overallAverage || 'MISSING'}`)) {
      testsFailed++;
      logError('Overall average not found. Check if Total row exists in table.');
    } else {
      testsPassed++;
    }

    if (!assert(parsed.summary?.overallAverage >= 1 && parsed.summary?.overallAverage <= 10, `Average in valid range (1-10): ${parsed.summary?.overallAverage}`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }

    if (!assert(typeof parsed.summary?.totalUnexcusedAbsences === 'number', `Unexcused absences: ${parsed.summary?.totalUnexcusedAbsences ?? 'MISSING'}`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }

    if (!assert(typeof parsed.summary?.totalExcusedAbsences === 'number', `Excused absences: ${parsed.summary?.totalExcusedAbsences ?? 'MISSING'}`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }
    ...
    if (!assert(parsed.subjects.length > 0, `Number of subjects: ${parsed.subjects.length}`)) {
      testsFailed++;
      logError('No subjects found. Check if #summary table exists and has rows.');
      throw new Error('No subjects parsed');
    }
    testsPassed++;

    if (!assert(parsed.subjects.length >= 10, `Subjects count reasonable (≥10): ${parsed.subjects.length}`)) {
      testsFailed++;
      logWarning('Fewer subjects than expected. Some may not be parsing correctly.');
    } else {
      testsPassed++;
    }
    ...
    if (!assert(firstSubject.name && firstSubject.name.length > 0, `First subject name: "${firstSubject.name}"`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }

    if (!assert(Array.isArray(firstSubject.grades), `First subject grades array: ${firstSubject.grades?.length || 0} grades`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }

    if (!assert(firstSubject.grades.length > 0, `First subject has grades: ${firstSubject.grades.length > 0 ? 'YES' : 'NO'}`)) {
      testsFailed++;
    ...
    if (firstSubject.average !== null) {
      if (!assert(firstSubject.average >= 1 && firstSubject.average <= 10, `First subject average valid: ${firstSubject.average}`)) {
        testsFailed++;
      } else {
        testsPassed++;
      }
    ...
    if (!assert(typeof firstSubject.unexcusedAbsences === 'number', `Unexcused absences type valid: ${typeof firstSubject.unexcusedAbsences}`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }

    if (!assert(typeof firstSubject.excusedAbsences === 'number', `Excused absences type valid: ${typeof firstSubject.excusedAbsences}`)) {
      testsFailed++;
    } else {
      testsPassed++;
    }
    ...
    parsed.subjects.slice(0, 3).forEach((subject, index) => {
      ...
      log(`     Average: ${subject.average ?? 'N/A'}, Absences: ${subject.unexcusedAbsences}/${subject.excusedAbsences}`, 'reset');
    });
